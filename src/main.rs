...
// ESP mount points and options
const ESP_MOUNT_POINTS: [&str; 3] = ["/boot", "/boot/efi", "/efi"];
const ESP_MOUNT_OPTIONS: [&str; 2] = ["fmask=0022", "dmask=0022"];
...
fn mount_block_device(
    ...
    if device.fs_type == "zfs_member" {
        let pool_name = device.zpool_name.as_ref().unwrap_or(&args.zfs_pool_name);
        
        if !zfs::import_pool(device, mount_point) {
            return false;
        }
        
        return zfs::mount_all_datasets(pool_name);
    }
    ...
    // Special handling for ESP partitions
    let is_esp = ESP_MOUNT_POINTS.iter().any(|&mp| mount_point.ends_with(mp)) 
        && device.fs_type == "vfat";
    let final_options = if is_esp {
        ESP_MOUNT_OPTIONS.iter().map(|&s| s.to_string()).collect()
    } else {
        options
    };
...
fn list_block_devices(ignored_devices: Option<Vec<BlockDevice>>) -> Vec<BlockDevice> {
    ...
    let mut disks: block_device::BlockDevices =  
        serde_json::from_str(&disks_raw).expect("Failed to parse lsblk output");

    // Add ZFS pool detection with CachyOS defaults
    for device in &mut disks.block_devices {
        if device.fs_type == "zfs_member" {
            // Try to get pool name from zdb
            let zpool_info = Exec::cmd("zdb")
                .arg("-l")
                .arg(&device.name)
                .capture()
                .ok()
            ...
            device.zpool_name = zpool_info.or(Some(args.zfs_pool_name.clone()));
        }
    }
...
fn main() {
    ...
    if !ideal_fstab_path.exists() {  
        ...
        for entry in &entries {  
            ...
            // Special handling for ESP partitions
            if ESP_MOUNT_POINTS.iter().any(|&mp| entry.mountpoint.to_str().unwrap() == mp) 
                && device.fs_type == "vfat" {
                if mount_block_device(
                    device,
                    actual_mount_point,
                    true,
                    Some(ESP_MOUNT_OPTIONS.iter().map(|&s| s.to_string()).collect()),
                ) {
    ...
    // Before unmounting, handle ZFS if needed
    if selected_device.fs_type == "zfs_member" {
        let pool_name = selected_device.zpool_name.as_ref().unwrap_or(&args.zfs_pool_name);
        zfs::unmount_all_datasets(pool_name);
